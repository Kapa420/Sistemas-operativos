...
void *function1(void *arg)
{
    ...
    while (arg1->flag == 0)
    {
        clock_gettime(CLOCK_REALTIME, &arg1->ts);
        arg1->ts.tv_sec += 1;

        if(!pthread_mutex_timedlock(&mutex, &arg1->ts))
        {
            for (int i = 0; i < 4; i++)
            {
                std::cout << arg1->sym;
...
void *function2(void *arg)
{
    ...
    while (arg2->flag == 0)
    {
        clock_gettime(CLOCK_REALTIME, &arg2->ts);
        arg2->ts.tv_sec += 1;
        if(!pthread_mutex_timedlock(&mutex, &arg2->ts))
        { 
            for (int i = 0; i < 4; i++)
            {
                std::cout << arg2->sym;
